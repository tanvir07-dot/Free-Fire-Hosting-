...
async def RejectMSGtaxt(squad_owner,uid, key, iv):
    random_banner = f"""
[B][C][00FF00] YOUTUBE & [B][C][0000FF]TIKTOK [B][C][FFFFFF]PLEASE [B][C][FF7F7F]SUBSCRIBE [B][C][FFFFFF]ME ❤️ 
            [B][C][00E5FF]@PSYCHOTANVIR07
...
async def send_room_chat_enhanced(Msg, room_id, key, iv, region):
    ...
    fields = {
        1: 1,
        2: {
            1: 9367174684,  # Sender UID (from leaked packet)
            2: int(room_id),
            ...
            7: 2,
            9: {
                1: "PSYCHO TANVIR07!ㅤ",  # Your bot name
